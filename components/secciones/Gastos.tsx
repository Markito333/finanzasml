...
export default function Gastos() {
  ...
  const [formCategoria, setFormCategoria] = useState<string>(CATEGORIAS_GASTO[0].label)
  const [formFecha, setFormFecha] = useState(hoyISO())

  function abrirNuevo() {
    setEditando(null); setFormDescripcion(''); setFormMonto('')
    setFormCuentaId(cuentas[0]?.id || ''); setFormCategoria(CATEGORIAS_GASTO[0].label); setFormFecha(hoyISO()); setModalAbierto(true)
  }
  ...
  function handleGuardar() {
    ...
    if (editando) { actualizarTransaccion(editando.id, d); mostrarToast('Gasto actualizado', 'success') }
    else { agregarTransaccion({ ...d, tipo: 'gasto' }); mostrarToast('Gasto registrado', 'success') }
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-zinc-800">Gastos</h2>
        <button onClick={abrirNuevo} disabled={cuentas.length === 0}
          className="px-4 py-2 bg-zinc-900 text-white text-sm font-medium rounded-xl hover:bg-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">+ Nuevo Gasto</button>
      </div>

      {cuentas.length === 0 ? (
        <EmptyState mensaje="Necesitas crear una cuenta antes de registrar gastos." accion="Ir a Cuentas" onAccion={() => {}} />
      ) : gastos.length === 0 ? (
        <EmptyState mensaje="No hay gastos registrados. ¡Agrega tu primer gasto!" accion="Nuevo Gasto" onAccion={abrirNuevo} />
      ) : (
        <>
          ...
          <div className="bg-white rounded-2xl border border-zinc-100/50 divide-y divide-zinc-50">
            {gastosFiltrados.length === 0 ? (
              <div className="p-8 text-center text-sm text-zinc-400">No hay gastos con los filtros seleccionados.</div>
            ) : (
              gastosFiltrados.map((g) => {
                ...
                return (
                  <div key={g.id} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50/50 transition-colors">
                    ...
                    <p className="text-sm font-semibold text-red-500">-{formatearMoneda(g.monto)}</p>
      ...
      <Modal abierto={modalAbierto} onCerrar={() => setModalAbierto(false)} titulo={editando ? 'Editar Gasto' : 'Nuevo Gasto'}>
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-zinc-500 mb-1.5">Descripción</label>
            <input type="text" value={formDescripcion} onChange={(e) => setFormDescripcion(e.target.value)} placeholder="Ej: Compra del supermercado"
          ...
          <div>
            ...
            <div className="relative">
              <select value={formCategoria} onChange={(e) => setFormCategoria(e.target.value)}
                className="w-full px-3.5 py-2.5 pr-10 text-sm bg-zinc-100 border-0 rounded-xl focus:outline-none focus:ring-2 focus:ring-zinc-900/10 appearance-none">
                {CATEGORIAS_GASTO.map((cat) => (<option key={cat.label} value={cat.label}>{cat.label}</option>))}
              </select>
              <ChevronDownIcon className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400 pointer-events-none" />
            </div>
          </div>
      ...
      <Modal abierto={confirmarEliminar !== null} onCerrar={() => setConfirmarEliminar(null)} titulo="Eliminar Gasto">
        <div className="space-y-4">
          <p className="text-sm text-zinc-600">¿Estás seguro de eliminar este gasto?</p>
          <div className="flex gap-2">
            ...
            <button onClick={() => { if (confirmarEliminar) { eliminarTransaccion(confirmarEliminar); mostrarToast('Gasto eliminado', 'error') }; setConfirmarEliminar(null) }}
